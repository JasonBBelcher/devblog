...
use an object literal. Defining an Entity class/interface helps with readability.

One important difference to note is that interfaces are stripped away at runtime. Classes are not.
...
Why do this? You could easily do all your business logic inside the component in this case but
rather than that lets show how to do this in a way that scales by using a service class.

Angular services are a great way to share functionality to other components or even different
modules. Right now we have no backend so all persistance will be done using [localStorage](https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage)
available in all modern browsers. Later this mocked service will be replaced with the api
service that will talk to the Node Express todo api.

I will write below inline comments of what each method implementation is doing for
those that need it.
...
export class RepositoryService {
  ...
  */
  ...
  /*
   ...
   update first then update it.  Array.prototype.findIndex
  ...
  */
  ...
  // reset the array to an empty array. Pretty straight forward.
...
the box syntax](https://blog.angulartraining.com/tutorial-create-your-own-two-way-data-binding-in-angular-46487650ea82) `[()]` for two data binding.
